...
test("returns 200 if admin on header", async () => {
  const response = await fetch("http://localhost:8083/api/admin", {
    headers: {
...
test("returns 200 if admin on query param", async () => {
  const response = await fetch(
    "http://localhost:8083/api/admin?api-key=developer-test-key"
  );
  expect(response.status).toEqual(200);
});
...
test("returns 403 if bad query param", async () => {
  const response = await fetch(
    "http://localhost:8083/api/admin?api-key=bad-key"
  );
  expect(response.status).toEqual(403);
});
